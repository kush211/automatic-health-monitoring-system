...
export function ReceptionistDashboard() {
  ...
  const todaysAppointments = useMemo(() => {
    ...
    const demoDate = new Date('2024-07-28T00:00:00Z');
    return appointments.filter((app) => isSameDay(new Date(app.dateTime), demoDate));
  }, [appointments]);
                       ...
                       filteredAppointments.map((app, index) => (
                        <TableRow key={app.appointmentId}>
                            ...
                            <TableCell className="text-right">
                                <Button variant="ghost" size="icon" onClick={() => handleOpenPrintToken(app)} >
